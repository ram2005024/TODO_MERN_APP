...
const Home = () => {
  ...
  const fetchData = async () => {
    const { data } = await axios.get("https://todo-mern-app-auqa.vercel.app/todo");
    setTasks(data.data);
    setCopy(data.data);
  };
  const handleAddTask = async () => {
    ...
    try {
      const { data } = await axios.post("https://todo-mern-app-auqa.vercel.app/todo", obj);
      if (data.success) {
    ...
    if (isEdit && text) {
      ...
      try {
        const { data } = await axios.put(
          `https://todo-mern-app-auqa.vercel.app/todo/${isEdit._id}`,
          obj
        );
        if (data.success) {
          toast.success(data.message);
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axios.delete("https://todo-mern-app-auqa.vercel.app/todo/" + id);
      if (data.success) {
        toast.success(data.message);
        setText("");
        setEdit(null);
  ...
  const handleCheckAndUpdate = async (id, nameOfTask, isDone) => {
    ...
    try {
      const { data } = await axios.put(`https://todo-mern-app-auqa.vercel.app/todo/${id}`, obj);
      if (data.success) {
        toast.success(data.message);
        fetchData();
      } else {
